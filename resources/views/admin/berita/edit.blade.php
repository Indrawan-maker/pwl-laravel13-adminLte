            ...
            <div class="card-body">
                <form action="{{ route('berita.update', $berita->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                      ...
                      @foreach ( $kategory as $kategory_item)
                        <option value="{{ $kategori_item->id}}"
                        {{ $berita->kategori_id == $kategori_item->id ? 'selected' : '' }}>
                          {{$kategory_item->nama_kategori}}
                        </option>
                        @endforeach
                      </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" value="{{ $berita->judul_berita }}" name="judul_berita" class="form-control">
                    </div>

                    <div class="mb-3">
                        <label for="">Isi berita</label>
                        <textarea type="textarea" name="isi_berita" class="form-control">{{ $berita->isi_berita }}</textarea>
                    </div>
